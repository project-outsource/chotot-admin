...
import { useGetPaymentTypeById } from '@/api/job/paymentType/queries';
import { Icons } from '@/assets/icons';
import FullScreenLoading from '@/components/FullScreenLoading';
import { Button } from '@/components/ui/button';
import { Dialog, DialogContent, DialogTrigger } from '@/components/ui/dialog';

import FormCareer from './FormPaymentType';

type UpdatePaymentTypeProps = {
  id: number;
};

const UpdatePaymentType = ({ id }: UpdatePaymentTypeProps) => {
  const [isModalOpen, setIsModalOpen] = useState<boolean>(false);

  const { data, isFetching } = useGetPaymentTypeById(id, { enabled: isModalOpen });
