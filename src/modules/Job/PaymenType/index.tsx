...
import { useDeletePaymentType } from '@/api/job/paymentType/mutations';
import { useGetAllPaymentTypes } from '@/api/job/paymentType/queries';
...
import FormPaymentType from './components/FormPaymentType';
import UpdateCareer from './components/UpdatePaymentType';

const PaymentType: NextPageWithLayout = () => {
  const defaultQuery = {
    page: 1,
    limit: 10,
    sort: 'updatedAt,DESC',
  ...
  const {
    ...
    isFetching,
    refetch,
  } = useGetAllPaymentTypes(paramsQuery, {
    onError: (error) => toast.error(error.message),
  });
  const { mutate: deletePaymentType } = useDeletePaymentType();

  const columns = ['#', 'id', 'Hình thức trả lương', 'Link', 'Hành động'];
  ...
  const handleDelete = (id: number) => {
    deletePaymentType(id, {
      onSuccess: () => {
        refetch();
        toast.success('Xóa hình thức trả lương thành công !');
      },
      onError: (error) => {
  ...
  return (
    <>
      ...
      <div>
        <HStack pos="apart">
          <h1 className="text-base font-bold lg:text-2xl">Các hình thức trả lương của hệ thống</h1>
          <Dialog open={isModalCreate} onOpenChange={setIsModalCreate}>
            ...
            <DialogContent className="border-main-40 w-full max-w-[680px]">
              <FormPaymentType setIsOpen={setIsModalCreate} mode="create" />
            </DialogContent>
          </Dialog>
        </HStack>
        ...
        <HStack className="my-5" pos="apart">
          <HStack className="" spacing={16}>
            <Chip>Tổng ({paymentTypes?.length || 0})</Chip>
          </HStack>
        ...
        <div className="min-h-[200px]">
          <Table>
            ...
            <TableBody>
              <Show when={!isFetching && !!paymentTypes}>
                {paymentTypes?.map((career, index) => (
          ...
          {!isFetching && !paymentTypes && <div className="mt-6 text-center">0 records on the system!!</div>}
        ...
        {/* <div className="mt-2">
          <Pagination
            ...
            totalCount={paymentTypes?.total || 0}
            currentPage={paymentTypes?.page || 0}
