...
import { useNewExperience, useUpdateExperience } from '@/api/job/experince/mutations';
...
const FormExperience = ({ mode, setIsOpen, defaultValues }: FormUserProp) => {
  ...
  const { mutate: createExperienceMutate, isLoading: isLoadingCreate } = useNewExperience();
  const { mutate: updateExperienceMutate, isLoading: isLoadingUpdate } = useUpdateExperience();
  ...
  const handleSubmit = (values: CareerSchema) => {
    ...
    if (mode === 'edit') {
      ...
      updateExperienceMutate(
        { experienceId: +defaultValues.id, experience: values },
        {
          onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['experience'] });
            toast.success('Chỉnh sửa kinh nghiệm thành công !');
            setIsOpen(false);
          },
          onError: (error) => {
  ...
  return (
    <div>
      <h3 className="text-main-50 mb-6 text-center text-xl font-bold">Tạo mới kinh nghiệm của hệ thống</h3>
