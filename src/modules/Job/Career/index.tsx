...
import { useDeleteCareer } from '@/api/job/career/mutations';
import { useGetAllCareerQuery } from '@/api/job/career/queries';
...
const CareerAdminPage: NextPageWithLayout = () => {
  ...
  const {
    ...
    isFetching,
    refetch,
  } = useGetAllCareerQuery(paramsQuery, {
    onError: (error) => toast.error(error.message),
  });
  const { mutate: deleCareer } = useDeleteCareer();
  ...
  return (
    <>
      ...
      <div>
        <HStack pos="apart">
          <h1 className="text-base font-bold lg:text-2xl">Các ngành nghề của hệ thống</h1>
        ...
        <HStack className="my-5" pos="apart">
          <HStack className="" spacing={16}>
            <Chip>Tổng ngành nghề ({careers?.length || 0})</Chip>
          </HStack>
