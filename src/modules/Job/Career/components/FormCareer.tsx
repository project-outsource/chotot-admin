...
import { useNewCareer, useUpdateCareer } from '@/api/job/career/mutations';
...
const FormCareer = ({ mode, setIsOpen, defaultValues }: FormUserProp) => {
  ...
  const { mutate: createCareerMutate, isLoading: isLoadingCreate } = useNewCareer();
  const { mutate: updateCareerMutate, isLoading: isLoadingUpdate } = useUpdateCareer();
  ...
  const handleSubmit = (values: CareerSchema) => {
    ...
    if (mode === 'edit') {
      ...
      updateCareerMutate(
        { careerId: +defaultValues.id, career: values },
        {
          onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['career'] });
