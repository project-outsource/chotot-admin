...
import { useNewJobType, useUpdateJobType } from '@/api/job/jobType/mutations';
...
const FormCareer = ({ mode, setIsOpen, defaultValues }: FormUserProp) => {
  ...
  const { mutate: createCareerMutate, isLoading: isLoadingCreate } = useNewJobType();
  const { mutate: updateCareerMutate, isLoading: isLoadingUpdate } = useUpdateJobType();
  ...
  const handleSubmit = (values: CareerSchema) => {
    if (mode === 'create') {
      createCareerMutate(values, {
        onSuccess: () => {
          queryClient.invalidateQueries({ queryKey: ['type-of-work'] });
    ...
    if (mode === 'edit') {
      ...
      updateCareerMutate(
        { jobTypeId: +defaultValues.id, jobType: values },
        {
          onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['type-of-work'] });
