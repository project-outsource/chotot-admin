...
import { useGetHouseCategoryById } from '@/api/house/category/queries';
import { Icons } from '@/assets/icons';
import FullScreenLoading from '@/components/FullScreenLoading';
import { Button } from '@/components/ui/button';
import { Dialog, DialogContent, DialogTrigger } from '@/components/ui/dialog';

import FormCareer from './FormCategory';

type UpdateCategoryProps = {
  id: number;
};

const UpdateCategory = ({ id }: UpdateCategoryProps) => {
  const [isModalOpen, setIsModalOpen] = useState<boolean>(false);

  const { data, isFetching } = useGetHouseCategoryById(id, { enabled: isModalOpen });
