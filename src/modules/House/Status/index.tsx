...
import { useDeleteHouseStatus } from '@/api/house/status/mutations';
import { useGetAllHouseStatusQuery } from '@/api/house/status/queries';
...
import FormCareer from './components/FormStatus';
import UpdateCareer from './components/UpdateStatus';
...
const CareerAdminPage: NextPageWithLayout = () => {
  ...
  const {
    ...
    isFetching,
    refetch,
  } = useGetAllHouseStatusQuery(paramsQuery, {
    onError: (error) => toast.error(error.message),
  });
  const { mutate: deleCareer } = useDeleteHouseStatus();
  ...
  return (
    <>
      ...
      <div>
        <HStack pos="apart">
          <h1 className="text-base font-bold lg:text-2xl">Các tình trạng nội thất của hệ thống</h1>
