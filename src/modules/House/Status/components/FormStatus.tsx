...
import { useNewHouseStatus, useUpdateHouseStatus } from '@/api/house/status/mutations';
...
const FormStatus = ({ mode, setIsOpen, defaultValues }: FormUserProp) => {
  ...
  const { mutate: createHouseStatusMutate, isLoading: isLoadingCreate } = useNewHouseStatus();
  const { mutate: updateHouseStatusMutate, isLoading: isLoadingUpdate } = useUpdateHouseStatus();
  const queryClient = useQueryClient();

  const handleSubmit = (values: CareerSchema) => {
    if (mode === 'create') {
      createHouseStatusMutate(values, {
        onSuccess: () => {
          queryClient.invalidateQueries({ queryKey: ['HouseStatus'] });
    ...
    if (mode === 'edit') {
      ...
      updateHouseStatusMutate(
        { houseStatusId: +defaultValues.id, houseStatus: values },
        {
          onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['HouseStatus'] });
