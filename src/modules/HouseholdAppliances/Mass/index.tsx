...
import { useDeleteHouseAppMass } from '@/api/household-appliances/mass/mutations';
import { useGetAllHouseAppMassQuery } from '@/api/household-appliances/mass/queries';
...
const HouseMassAdminPage: NextPageWithLayout = () => {
  const defaultQuery = {
    page: 1,
    limit: 10,
    sort: 'updatedAt,DESC',
  ...
  const {
    ...
    isFetching,
    refetch,
  } = useGetAllHouseAppMassQuery(paramsQuery, {
    onError: (error) => toast.error(error.message),
  });
  const { mutate: deleCareer } = useDeleteHouseAppMass();

  const columns = ['#', 'id', 'Tên khối lượng', 'Link', 'Hành động'];
  ...
  return (
    <>
      ...
      <div>
        <HStack pos="apart">
          <h1 className="text-base font-bold lg:text-2xl">Các khối lượng của hệ thống</h1>
        ...
        <HStack className="my-5" pos="apart">
          <HStack className="" spacing={16}>
            <Chip>Tổng khối lượng ({careers?.length || 0})</Chip>
          </HStack>
...
export default HouseMassAdminPage;
