...
import { useNewHouseAppMass, useUpdateHouseAppMass } from '@/api/household-appliances/mass/mutations';
...
const FormExperience = ({ mode, setIsOpen, defaultValues }: FormUserProp) => {
  ...
  const { mutate: createExperienceMutate, isLoading: isLoadingCreate } = useNewHouseAppMass();
  const { mutate: updateExperienceMutate, isLoading: isLoadingUpdate } = useUpdateHouseAppMass();
  ...
  const handleSubmit = (values: CareerSchema) => {
    if (mode === 'create') {
      createExperienceMutate(values, {
        onSuccess: () => {
          queryClient.invalidateQueries({ queryKey: ['HouseAppMass'] });
          toast.success('Tạo khối lượng thành công !');
          setIsOpen(false);
        },
        onError: (error) => {
    ...
    if (mode === 'edit') {
      ...
      updateExperienceMutate(
        { houseAppMassId: +defaultValues.id, houseAppMass: values },
        {
          onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['HouseAppMass'] });
