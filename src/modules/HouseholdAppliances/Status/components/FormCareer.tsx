...
import { useNewHouseAppStatus, useUpdateHouseAppStatus } from '@/api/household-appliances/status/mutations';
...
const FormExperience = ({ mode, setIsOpen, defaultValues }: FormUserProp) => {
  ...
  const { mutate: createExperienceMutate, isLoading: isLoadingCreate } = useNewHouseAppStatus();
  const { mutate: updateExperienceMutate, isLoading: isLoadingUpdate } = useUpdateHouseAppStatus();
  ...
  const handleSubmit = (values: CareerSchema) => {
    if (mode === 'create') {
      createExperienceMutate(values, {
        onSuccess: () => {
          queryClient.invalidateQueries({ queryKey: ['HouseAppStatus'] });
    ...
    if (mode === 'edit') {
      ...
      updateExperienceMutate(
        { houseAppStatusId: +defaultValues.id, houseAppStatus: values },
        {
          onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['HouseAppStatus'] });
