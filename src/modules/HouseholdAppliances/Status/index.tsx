...
import { useDeleteHouseAppStatus } from '@/api/household-appliances/status/mutations';
import { useGetAllHouseAppStatusQuery } from '@/api/household-appliances/status/queries';
...
const HouseStatusAdminPage: NextPageWithLayout = () => {
  const defaultQuery = {
    page: 1,
    limit: 10,
    sort: 'updatedAt,DESC',
  ...
  const {
    ...
    isFetching,
    refetch,
  } = useGetAllHouseAppStatusQuery(paramsQuery, {
    onError: (error) => toast.error(error.message),
  });
  const { mutate: deleCareer } = useDeleteHouseAppStatus();

  const columns = ['#', 'id', 'Tên tình trạng', 'Link', 'Hành động'];
  ...
  const handleDelete = (id: number) => {
    deleCareer(id, {
      onSuccess: () => {
        refetch();
        toast.success('Xóa tình trạng thành công !');
      },
      onError: (error) => {
  ...
  return (
    <>
      ...
      <div>
        <HStack pos="apart">
          <h1 className="text-base font-bold lg:text-2xl">Các tình trạng của hệ thống</h1>
        ...
        <HStack className="my-5" pos="apart">
          <HStack className="" spacing={16}>
            <Chip>Tổng tình trạng ({careers?.length || 0})</Chip>
          </HStack>
...
export default HouseStatusAdminPage;
