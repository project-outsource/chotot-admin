...
import { useNewHouseAppWattage, useUpdateHouseAppWattage } from '@/api/household-appliances/wattage/mutations';
...
const FormExperience = ({ mode, setIsOpen, defaultValues }: FormUserProp) => {
  ...
  const { mutate: createExperienceMutate, isLoading: isLoadingCreate } = useNewHouseAppWattage();
  const { mutate: updateExperienceMutate, isLoading: isLoadingUpdate } = useUpdateHouseAppWattage();
  ...
  const handleSubmit = (values: CareerSchema) => {
    if (mode === 'create') {
      createExperienceMutate(values, {
        onSuccess: () => {
          ...
          toast.success('Tạo công suất thành công !');
          setIsOpen(false);
        },
        onError: (error) => {
    ...
    if (mode === 'edit') {
      ...
      updateExperienceMutate(
        { houseAppWattageId: +defaultValues.id, houseAppWattage: values },
        {
          onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['career'] });
            toast.success('Chỉnh sửa công suất thành công !');
            setIsOpen(false);
          },
          onError: (error) => {
