...
import { useDeleteHouseAppWattage } from '@/api/household-appliances/wattage/mutations';
import { useGetAllHouseAppWattageQuery } from '@/api/household-appliances/wattage/queries';
...
const HouseWattageAdminPage: NextPageWithLayout = () => {
  const defaultQuery = {
    page: 1,
    limit: 10,
    sort: 'updatedAt,DESC',
  ...
  const {
    ...
    isFetching,
    refetch,
  } = useGetAllHouseAppWattageQuery(paramsQuery, {
    onError: (error) => toast.error(error.message),
  });
  const { mutate: deleCareer } = useDeleteHouseAppWattage();

  const columns = ['#', 'id', 'Tên công suất', 'Link', 'Hành động'];
  ...
  const handleDelete = (id: number) => {
    deleCareer(id, {
      onSuccess: () => {
        refetch();
        toast.success('Xóa công suất thành công !');
      },
      onError: (error) => {
  ...
  return (
    <>
      ...
      <div>
        <HStack pos="apart">
          <h1 className="text-base font-bold lg:text-2xl">Các công suất của hệ thống</h1>
        ...
        <HStack className="my-5" pos="apart">
          <HStack className="" spacing={16}>
            <Chip>Tổng công suất ({careers?.length || 0})</Chip>
          </HStack>
...
export default HouseWattageAdminPage;
