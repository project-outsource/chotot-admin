...
import { useDeleteHouseAppVolume } from '@/api/household-appliances/volume/mutations';
import { useGetAllHouseAppVolumeQuery } from '@/api/household-appliances/volume/queries';
...
const HouseVolumeAdminPage: NextPageWithLayout = () => {
  const defaultQuery = {
    page: 1,
    limit: 10,
    sort: 'updatedAt,DESC',
  ...
  const {
    ...
    isFetching,
    refetch,
  } = useGetAllHouseAppVolumeQuery(paramsQuery, {
    onError: (error) => toast.error(error.message),
  });
  const { mutate: deleCareer } = useDeleteHouseAppVolume();

  const columns = ['#', 'id', 'Tên thể tích', 'Link', 'Hành động'];
  ...
  const handleDelete = (id: number) => {
    deleCareer(id, {
      onSuccess: () => {
        refetch();
        toast.success('Xóa thể tích thành công !');
      },
      onError: (error) => {
  ...
  return (
    <>
      ...
      <div>
        <HStack pos="apart">
          <h1 className="text-base font-bold lg:text-2xl">Các thể tích của hệ thống</h1>
        ...
        <HStack className="my-5" pos="apart">
          <HStack className="" spacing={16}>
            <Chip>Tổng thể tích ({careers?.length || 0})</Chip>
          </HStack>
...
export default HouseVolumeAdminPage;
