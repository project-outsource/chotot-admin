...
import { useDeleteHouseAppCategory } from '@/api/household-appliances/category/mutations';
import { useGetAllHouseAppCategoryQuery } from '@/api/household-appliances/category/queries';
...
const HouseCategoryAdminPage: NextPageWithLayout = () => {
  const defaultQuery = {
    page: 1,
    limit: 10,
    sort: 'updatedAt,DESC',
  ...
  const {
    ...
    isFetching,
    refetch,
  } = useGetAllHouseAppCategoryQuery(paramsQuery, {
    onError: (error) => toast.error(error.message),
  });
  const { mutate: deleCareer } = useDeleteHouseAppCategory();

  const columns = ['#', 'id', 'Tên danh mục', 'Link', 'Hành động'];
  ...
  return (
    <>
      ...
      <div>
        <HStack pos="apart">
          <h1 className="text-base font-bold lg:text-2xl">Các danh mục của hệ thống</h1>
        ...
        <HStack className="my-5" pos="apart">
          <HStack className="" spacing={16}>
            <Chip>Tổng danh mục ({careers?.length || 0})</Chip>
          </HStack>
...
export default HouseCategoryAdminPage;
