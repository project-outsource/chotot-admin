...
import { useDeleteHouseAppGuarantee } from '@/api/household-appliances/guaranteed/mutations';
import { useGetAllHouseAppGuaranteeQueryQuery } from '@/api/household-appliances/guaranteed/queries';
...
const HouseGuaranteeAdminPage: NextPageWithLayout = () => {
  const defaultQuery = {
    page: 1,
    limit: 10,
    sort: 'updatedAt,DESC',
  ...
  const {
    ...
    isFetching,
    refetch,
  } = useGetAllHouseAppGuaranteeQueryQuery(paramsQuery, {
    onError: (error) => toast.error(error.message),
  });
  const { mutate: deleCareer } = useDeleteHouseAppGuarantee();

  const columns = ['#', 'id', 'Tên hình thức bảo hành', 'Link', 'Hành động'];
  ...
  const handleDelete = (id: number) => {
    deleCareer(id, {
      onSuccess: () => {
        refetch();
        toast.success('Xóa hình thức bảo hành thành công !');
      },
      onError: (error) => {
  ...
  return (
    <>
      ...
      <div>
        <HStack pos="apart">
          <h1 className="text-base font-bold lg:text-2xl">Các hình thức bảo hành của hệ thống</h1>
        ...
        <HStack className="my-5" pos="apart">
          <HStack className="" spacing={16}>
            <Chip>Tổng hình thức bảo hành ({careers?.length || 0})</Chip>
          </HStack>
...
export default HouseGuaranteeAdminPage;
