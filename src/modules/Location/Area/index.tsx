...
import { useDeleteLocation } from '@/api/location/area/mutations';
...
import FormCareer from './components/FormArea';
import UpdateCareer from './components/UpdateArea';
...
const CareerAdminPage: NextPageWithLayout = () => {
  ...
  const {
    ...
    isFetching,
    refetch,
  } = useGetAllLocationQuery(paramsQuery, {
    onError: (error) => toast.error(error.message),
  });
  const { mutate: deleCareer } = useDeleteLocation();

  const columns = ['#', 'id', 'Tên khu vực', 'Link', 'Hành động'];
  ...
  return (
    <>
      ...
      <div>
        <HStack pos="apart">
          <h1 className="text-base font-bold lg:text-2xl">Các khu vực của hệ thống</h1>
        ...
        <HStack className="my-5" pos="apart">
          <HStack className="" spacing={16}>
            <Chip>Tổng khu vực ({careers?.length || 0})</Chip>
          </HStack>
