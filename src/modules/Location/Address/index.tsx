...
import { useDeleteLocationAddress } from '@/api/location/address/mutations';
import { useGetAllLocationAddressQuery } from '@/api/location/address/queries';
...
import FormCareer from './components/FormAddress';
import UpdateCareer from './components/UpdateAddress';
...
const CareerAdminPage: NextPageWithLayout = () => {
  ...
  const {
    ...
    isFetching,
    refetch,
  } = useGetAllLocationAddressQuery(paramsQuery, {
    onError: (error) => toast.error(error.message),
  });
  const { mutate: deleCareer } = useDeleteLocationAddress();

  const columns = ['#', 'id', 'Tên địa chỉ', 'Tên quốc tế', 'Link', 'Hành động'];
  ...
  return (
    <>
      ...
      <div>
        <HStack pos="apart">
          <h1 className="text-base font-bold lg:text-2xl">Các địa chỉ của hệ thống</h1>
        ...
        <HStack className="my-5" pos="apart">
          <HStack className="" spacing={16}>
            <Chip>Tổng địa chỉ ({careers?.length || 0})</Chip>
          </HStack>
              ...
              <Show when={!isFetching && !!careers}>
                {careers?.map((career, index) => (
                  <TableRow key={index} className="bg-white">
                    ...
                    <TableCell className="text-center">{career.Name_en}</TableCell>
