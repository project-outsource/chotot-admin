...
import { useNewSpecialStatus, useUpdateSpecialStatus } from '@/api/specialiSao-equipment/status/mutations';
...
import { careerSchema } from '@/validations/job/career';
...
const FormExperience = ({ mode, setIsOpen, defaultValues }: FormUserProp) => {
  ...
  const { mutate: createExperienceMutate, isLoading: isLoadingCreate } = useNewSpecialStatus();
  const { mutate: updateExperienceMutate, isLoading: isLoadingUpdate } = useUpdateSpecialStatus();
  ...
  const handleSubmit = (values: CareerSchema) => {
    if (mode === 'create') {
      createExperienceMutate(values, {
        onSuccess: () => {
          queryClient.invalidateQueries({ queryKey: ['specialStatus'] });
          toast.success('Tạo tình trạng thành công !');
          setIsOpen(false);
        },
        onError: (error) => {
    ...
    if (mode === 'edit') {
      ...
      updateExperienceMutate(
        { specialStatusId: +defaultValues.id, specialStatus: values },
        {
          onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['SpecialStatus'] });
            toast.success('Chỉnh sửa tình trạng thành công !');
            setIsOpen(false);
          },
          onError: (error) => {
