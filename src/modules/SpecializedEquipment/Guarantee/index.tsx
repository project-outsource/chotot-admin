...
import { useGetAllSpecialGuaranteeQuery } from '@/api/specialiSao-equipment/guarantee/queries';
import { useDeleteSpecialStatus } from '@/api/specialiSao-equipment/status/mutations';
...
const CareerAdminPage: NextPageWithLayout = () => {
  ...
  const {
    ...
    isFetching,
    refetch,
  } = useGetAllSpecialGuaranteeQuery(paramsQuery, {
    onError: (error) => toast.error(error.message),
  });
  const { mutate: deleCareer } = useDeleteSpecialStatus();

  const columns = ['#', 'id', 'Tên thông tin bảo hànhhành', 'Link', 'Hành động'];
  ...
  const handleDelete = (id: number) => {
    deleCareer(id, {
      onSuccess: () => {
        refetch();
        toast.success('Xóa thông tin bảo hànhhành thành công !');
      },
      onError: (error) => {
  ...
  return (
    <>
      ...
      <div>
        <HStack pos="apart">
          <h1 className="text-base font-bold lg:text-2xl">Các thông tin bảo hànhhành của hệ thống</h1>
        ...
        <HStack className="my-5" pos="apart">
          <HStack className="" spacing={16}>
            <Chip>Tổng thông tin bảo hànhhành ({careers?.length || 0})</Chip>
          </HStack>
